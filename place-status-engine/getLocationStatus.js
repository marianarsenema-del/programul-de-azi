...
async function getLocationStatus({ pool, placeId, apiKey, language, cacheOnly, ttlHours, checkFetchAllowed }) {
  ...
  if (!raw) {
    ...
    fromCache = false;
    // Apelul plătit către Google — singurul loc din tot fișierul care NU
    // era protejat. Bug real, prins direct din producție: o singură cotă
    // depășită (sau orice altă eroare temporară de la Google) arunca o
    // excepție necapturată care trecea prin tot lanțul de apeluri și
    // dobora pagina ÎNTREAGĂ (ex. /cazare) pentru orice vizitator — nu
    // doar afișa lipsă statusul pentru o singură locație. Acum tratăm la
    // fel ca "skipped" mai sus — pagina rămâne funcțională, doar acel
    // indicator de status lipsește pentru locația respectivă.
    try {
      raw = await fetchPlaceDetails(placeId, apiKey, googleLang);
      await saveCachedDetails(pool, placeId, googleLang, raw);
    } catch (err) {
      return { skipped: true, error: true, fromCache: false };
    }
  }
