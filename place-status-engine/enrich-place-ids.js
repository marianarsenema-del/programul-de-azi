  ...
  while (true) {
    ...
    for (const row of rows) {
      ...
      if (result && result.status === "OK") {
        ...
        // IMPORTANT: marcăm cu o valoare specială, NU lăsăm place_id gol —
        // altfel rândul rămâne mereu eligibil pentru "WHERE place_id IS NULL"
        // și e reluat la nesfârșit, ÎN ACEEAȘI rulare, de îndată ce toate
        // celelalte rânduri "găsibile" au fost procesate (buclă fără capăt,
        // cereri plătite irosite către Google, la infinit). Cu marcajul
        // "ZERO_RESULTS", rândul e exclus corect din interogările viitoare.
        await pool.query(`UPDATE ${DB_TABLE} SET place_id = $1 WHERE id = $2`, [
          "ZERO_RESULTS",
          row.id,
        ]);
        totalNotFound += 1;
        console.warn(`${label} -> niciun rezultat găsit (ZERO_RESULTS) — marcat, nu va mai fi reluat automat`);
      } else if (result) {
        // Orice alt răspuns DEFINITIV de la Google (REQUEST_DENIED,
        // INVALID_REQUEST etc.) — marcăm și pe astea, din același motiv ca
        // mai sus. Doar erorile de REȚEA (fetch eșuat complet, result=null,
        // vezi catch-ul de mai sus) rămân eligibile pentru reîncercare
        // automată, pentru că alea chiar pot fi trecătoare (net picat o clipă).
        await pool.query(`UPDATE ${DB_TABLE} SET place_id = $1 WHERE id = $2`, [
        ...
        totalErrors += 1;
        console.error(
          `${label} -> ${result.status}${result.errorMessage ? " — " + result.errorMessage : ""} — marcat, nu va mai fi reluat automat`
        );
      }
